...
/// set per spawn by the fixture's own `Command` builder rather than baked
/// into the script bytes — see [`shared_fixture`]'s doc for why.
...
mod shared_fixture_proof {
    use super::{Fixture, install_fake_harness};
    ...
    /// this file that spawns the harness through `Fixture` goes through
    /// `Fixture::with_extra_config`, which unconditionally calls
    ...
    /// reads is exactly the one `Fixture::glasshouse` sets per spawn —
...
impl TwoHarnessFixture {
    fn new() -> Self {
        ...
        std::fs::create_dir_all(&bin_dir).expect("create bin dir");
        let argv_log = base.join("argv.log");
        // Same script body for both names (it only reads ARGV_LOG_VAR), so
        // `shared_fixture`'s content-keyed cache intentionally collapses
        // both onto ONE file — see the module doc and the report's account
        // of why that is behavior-preserving here (this fixture never reads
        // `argv_log` back, so which literal executable ran is not observed).
        let claude_code = install_named_fake_harness(&bin_dir, "fake-claude-code");
        let codex = install_named_fake_harness(&bin_dir, "fake-codex");
...
#[cfg(unix)]
fn install_named_fake_harness(_bin_dir: &Path, name: &str) -> PathBuf {
    shared_fixture(
        name,
        &format!("#!/bin/sh\nprintf '%s\\n' \"$*\" >> \"${ARGV_LOG_VAR}\"\nexit 0\n"),
    )
