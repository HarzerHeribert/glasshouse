...
//!
//! `GH-RESPONSIVENESS-TERMS`'s own section — `session.rs`'s sibling
//! "observed pairing reliability" term, reachable only through
//! `SessionRouter::choose` — went with the router (design-decisions.md,
//! 2026-09-16, "Glasshouse never decides which model is used"). It was a
//! distinct mechanism from `routing::interactive::score_candidate` above,
//! which stays: the gateway's own same-model failover ranking, untouched
//! by the ruling.
