...
fn a_task_naming_no_observed_file_adds_no_section() {
    ...
    let result = briefing(&store, "fix the bug", &HashSet::new(), None, None)
        .unwrap()
        .into_injection();
    assert!(
        result.is_none(),
...
fn a_task_naming_more_paths_than_the_bound_never_reaches_the_one_named_last() {
    ...
    let result = briefing(&store, &task, &HashSet::new(), None, None)
        .unwrap()
        .into_injection();
...
fn the_file_observed_records_for_an_in_bounds_task_keep_the_old_order_and_count() {
    ...
    let injection = briefing(&store, task, &HashSet::new(), None, None)
        .unwrap()
        .into_injection()
