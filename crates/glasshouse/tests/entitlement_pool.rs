...
/// The env vars the shared fixture script reads its argv- and env-dump
/// destinations from, set per spawn by [`Binary::glasshouse`] rather than
...
/// GH-ARGV-LOG-HOIST. The argv- and env-dump destinations used to be
/// interpolated into the script bytes, which made every call's content
/// distinct; they are now read from `ARGV_LOG_VAR` / `ENV_LOG_VAR` at spawn
/// time (set by [`Binary::glasshouse`]), so the script bytes are constant
/// and every call below collapses onto the one file the first caller writes.
///
...
mod shared_fixture_proof {
    ...
    /// `ARGV_LOG_VAR` and `ENV_LOG_VAR` are read only by the byte-for-byte fixture check in this
    ...
    #[cfg(unix)]
    use super::{ARGV_LOG_VAR, ENV_LOG_VAR};
    ...
    /// **Bytes constant.** The shared fixture's bytes read the argv- and
    /// env-dump destinations from `ARGV_LOG_VAR` / `ENV_LOG_VAR` rather than
    /// embedding per-test paths, so the script text is the same regardless
    /// of which tempdir asked for it.
    #[cfg(unix)]
    #[test]
    fn the_shared_fixture_reads_its_log_paths_from_the_env_vars_not_the_script() {
        let tmp = tempfile::tempdir().expect("tempdir");
        ...
        assert_eq!(
            ...
            "the shared fixture's bytes must read the log destinations from the env vars, \
             not have paths baked in"
        );
    }

    /// **End-to-end, through the real caller.** The env vars the fixture
    /// reads are exactly the ones [`Binary::glasshouse`] sets per spawn —
    /// proven by actually launching the shipped binary and reading both logs
    /// back, not by inspecting the script text alone.
    #[test]
    fn a_real_launch_through_the_shared_fixture_writes_both_logs_to_the_requested_paths() {
        ...
        assert!(
            !binary.child_environment().is_empty(),
