...
//!   approval selection, and an optional named response preset. Nothing about
//!   it has touched a real adapter yet.
//! - A [`LaunchOverlay`] is the **ephemeral, per-launch result** of asking one
//!   [`HarnessAdapter`] whether a profile can actually be honoured. It
//!   applies to exactly one child process and is consumed by [`LaunchOverlay::apply`].
...
//!
//! This module never imports `crate::config` or `crate::database`; [`resolve`]
//! is the only place a [`crate::secret::Secret`] exists.
// History: design-decisions.md, "Trims: profile/mod.rs", module doc.
...
pub struct LaunchProfile {
    ...
    /// communication policy. A name, not a resolved
    /// [`response::ResponseProfile`] — resolving one against
    /// `response::presets()` is cheap and total, so nothing is gained by
    /// resolving it before handing the profile over.
    ///
    ...
    // History: design-decisions.md, "Trims: profile/mod.rs", `response_preset` field doc.
...
impl LaunchOverlay {
    ...
    ///
    /// Separate from [`resolve`] and [`LaunchOverlay::apply`]: resolution
    /// happens before a session record exists, so there is no session
    /// directory yet to write into or point an environment variable at. This
    /// step fills in the [`crate::harness::ConfigPathPlacement`] the adapter
    /// declared once the caller knows where the session lives.
    ///
    /// The returned [`EphemeralConfigs`] removes every file it wrote when it
    /// drops and registers a [`crate::shutdown::on_forced_exit`] cleanup,
    /// because the forced exit path calls [`std::process::exit`] and runs no
    /// destructor. An overlay with nothing to write returns a guard that owns
    /// nothing, so a caller never has to ask whether it has any.
    // History: design-decisions.md, "Trims: profile/mod.rs", `fn install`.
...
/// [`Resolution`]: this is a property of *this gateway-backed call*, not of
/// the profile or the adapter.
///
...
/// `crate::config` (see the module documentation).
// History: design-decisions.md, "Trims: profile/mod.rs", `GatewayPairing` doc.
#[derive(Debug, Clone)]
...
/// see [`crate::config::EffectiveConfig::bypass_acknowledged`].
///
...
///
/// The approval arm is keyed on the **backend**, not the harness, because an
/// automatic-review mode is a server-side capability the backend may not
/// serve: [`ApprovalSelection::Default`] withholds it silently on a
/// non-native backend, [`ApprovalSelection::AutomaticReview`] is refused
/// there, and [`ApprovalSelection::Bypass`] is unaffected.
///
/// [`BackendResource::Native`] behaviour does not change by one byte.
// History: design-decisions.md, "Trims: profile/mod.rs", `fn resolve`.
...
/// *process* the caller started. `None` here is not "no gateway configured";
/// it is "this call site has no gateway to give", and a gateway-backed
/// profile refuses with [`Refusal::GatewayNotRunning`] rather than being
/// silently resolved against something else.
///
/// A gateway-backed profile resolves into exactly what a direct-provider
/// profile does, through the same adapter method, with two substitutions:
/// the base URL is the gateway's own loopback address, and the credential
/// written into the child is the **gateway's token** rather than any
/// provider key — the provider credential stays in this process.
///
/// `pairing` is [`GatewayPairing::default`] for every caller with no
/// [`crate::config::EffectiveConfig`] to resolve one from.
// History: design-decisions.md, "Trims: profile/mod.rs", `fn resolve_with_gateway`.
...
/// usable. [`BackendResource::Native`] is unaffected.
///
/// [`resolve`] and [`resolve_with_gateway`] stay pure functions of
/// [`Resolution`] — no real `PATH` lookup as a side effect of resolving a
/// profile whose caller never asked for one. A caller that has already
/// resolved the executable should hand back the
/// [`crate::harness::ExecutablePresence::Usable`] it already established
/// rather than pay for a second search; one that has not should call
/// [`crate::harness::ExecutablePresence::detect`] itself.
///
/// Always resolves with [`GatewayPairing::default`] — this entry point has
/// no production caller today.
// History: design-decisions.md, "Trims: profile/mod.rs", `fn resolve_checked`.
...
///
/// What makes this different from a direct provider:
///
/// 1. the base URL is the gateway's own loopback address rather than a
///    provider's;
/// 2. the credential written into the child is the gateway's token, already
///    in memory, so **no [`crate::secret::Secret`] is resolved here at all**;
...
///
/// Choosing a backend here is legitimate because the choice is **recorded**
/// as a [`crate::routing::interactive::Assignment`] the moment a profile
/// binds a session, reported in the launch's mechanism note, pinnable by the
/// user (line 518), migratable at a task boundary (line 511), and every
/// change is recorded with its cache consequence (lines 515, 516). The order
...
/// which is what makes Phase 9I line 537's per-key rotation possible. A
/// provider is a candidate if it serves at least one ingress protocol with a
/// base URL, and it gets a [`Route`] for **each** protocol it serves.
/// Every credential is resolved here, once, and moved into the [`Upstream`]
/// — unlike [`resolve`]'s direct-provider path, they stay for the gateway's
/// lifetime. `free` answers, by name, whether the caller marked a provider
/// free-tier; unasked answers `false`, [`Cost::Metered`]'s fail-closed default.
// History: design-decisions.md, "Trims: profile/mod.rs", `fn gateway_upstream`.
...
/// through a mechanism this crate never sees the credential or base URL for.
/// A [`BackendResource::GlasshouseGateway`] profile talks to Glasshouse's own
/// local listener; which upstream provider answers behind it is Phase 9H's
/// sticky-routing decision, made per session, not by this profile — so both
/// answer `Unavailable`.
///
/// A resolvable [`BackendResource::DirectProvider`] is always available:
/// once a protocol and base URL can be chosen at all,
/// [`crate::provider::discovery::ProbeTarget::BaseUrl`] is always a valid
/// target, even with no established model-list endpoint. The credential is
/// resolved the same way `apply_direct_provider` does, but unlike there a
/// probe with none is still built — a missing credential is still
/// information a report can use.
// History: design-decisions.md, "Trims: profile/mod.rs", `fn capability_probe`.
...
/// The timeout budget a pre-flight check runs under — deliberately not
/// [`crate::provider::discovery::ProbeTimeouts::default`].
///
/// A pre-flight check sits between `glasshouse launch` and the session and
/// nobody asked for it, so it may not cost what an interactive keystroke's
/// probe can afford: four seconds, once, and only for a profile that has a
/// check at all. Every provider catalogue probed on 2026-08-26 answered in
/// well under a second, so this is still an order of magnitude of headroom
/// over any measured healthy answer.
// History: design-decisions.md, "Trims: profile/mod.rs", `PREFLIGHT_TIMEOUTS`.
...
///
/// There is no `Refuse` variant: this check *reports*, and the launch
/// proceeds. No outcome of it is unambiguous evidence the combination is
/// wrong (a `404`/`405` from a base URL with no `GET` is the healthy
/// answer), reachability is not correctness (some hosts answer identically
/// to a real path and a nonsense one), a wrong combination costs one harness
/// startup while a false refusal costs the user the ability to start work at
/// all, and [`resolve`] already owns refusal — deterministically, offline,
/// from declarations.
///
...
// History: design-decisions.md, "Trims: profile/mod.rs", `Preflight` doc.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Preflight {
...
///
/// [`resolve`] and [`resolve_with_gateway`] stay pure functions of
/// [`Resolution`]: nothing here is called by either of them, and this runs
/// after resolution, never before it — a check that ran first and reached
/// `resolve` would be a router.
///
/// It costs nothing for a profile with no check available — every `Native`
/// and gateway-backed profile. For a direct provider, it costs exactly one
/// bounded HTTP request; see [`PREFLIGHT_TIMEOUTS`] for the ceiling.
///
/// The summary is built from names and [`describe_probe_outcome`], never the
/// credential, then passed through [`crate::secret::redact`] anyway because a
/// *base URL* is user-supplied text that can carry anything.
// History: design-decisions.md, "Trims: profile/mod.rs", `fn preflight`.
