...
/// [`super::EvidenceLedger::summarize`] answers anything but "unknown" for it —
...
/// [`super::EvidenceLedger::request_stats_by_harness`] and
...
/// it, and [`super::EvidenceLedger::classification_record`] reads it back.
///
...
/// [`super::RoutingOverhead`] reads it back as its own bucket.
///
...
/// writes it.
///
/// Spelled here rather than only at that producer so [`super::RoutingOverhead`] can
...
/// back by [`super::RoutingOverhead`] into its own bucket: a movement row is not a
...
/// How far back [`super::EvidenceLedger::classification_record`] and the routing
...
/// [`super::RoutingOverhead::from_consumption`] skips it by name, and
/// [`super::correlate_routes`] — which would otherwise read its own consequence
...
/// moment* for [`super::correlate_routes`] — capability map line 1370's
...
/// The fraction of task spend above which [`super::RoutingOverhead::exceeds`] says
...
impl FailureClass {
    ...
    /// [`super::correlate_routes`] matches on, capability map line 1373.
    ///
...
///
/// Counts, not rates, so unlike [`super::RoutingSummary`]'s aggregates they are not
...
impl NewObservation {
    ...
    /// [`crate::routing::burn::task_class_request_rates`] counts only rows that name
...
impl RoutingObservation {
    ...
    /// method — [`super::EvidenceLedger::classification_record`],
    /// [`super::EvidenceLedger::support_work_latency`] and the medians they compute
    ...
    /// [`super::ObservedIdentity::window`] return — and the interval
    /// [`super::correlate_routes`] tests for overlap.
    ///
...
impl<T> AggregateReading<T> {
    ...
    /// ever wrapped in one; see [`super::RoutingSummary`]'s own `Option` fields.
