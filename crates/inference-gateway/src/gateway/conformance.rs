...
use super::{Gateway, GatewayToken, Observation, ObservationSink};
...
fn gateway_to_with_health_cache(fixture: &FixtureUpstream, cache: GatewayHealthCache) -> Gateway {
    Gateway::start_with_telemetry(upstream_to(fixture), None, Some(cache))
        .expect("loopback is bindable")
}
...
// --- Phase 33A: the routing observation's gateway producer -----------------

/// The sink a gateway reports through, and what it reported.
///
/// The gateway keeps no ledger and opens no database — an observation leaves
/// this process and is somebody else's to store — so a conformance test
/// asserts on what came out of the sink, which is the whole of what a host
/// would ever see.
...
/// Every [`Observation::Routed`] reported so far, in the order it was
/// reported.
fn reported_observations(
    seen: &Arc<Mutex<Vec<Observation>>>,
) -> Vec<crate::routing::evidence::NewObservation> {
    seen.lock()
        .expect("no test panics while holding this")
        .iter()
        .filter_map(|reported| match reported {
            Observation::Routed { observation, .. } => Some(observation.as_ref().clone()),
            Observation::Degraded { .. } => None,
        })
        .collect()
}

/// [`gateway_to`], reporting every bound, provider-reaching exchange to
/// `sink` — the producer half of Phase 33A, proven the way every other
...
fn gateway_to_with_observation_sink(fixture: &FixtureUpstream, sink: ObservationSink) -> Gateway {
    Gateway::start_with_degrade_sink(upstream_to(fixture), None, None, Some(sink), None)
        .expect("loopback is bindable")
}
...
#[test]
fn a_real_forwarded_exchange_is_reported_to_the_sink() {
    use crate::routing::AssignedModel;
    use crate::routing::evidence::{HARNESS_TURN_PURPOSE, Outcome};
    ...
    // The report happens on the connection thread after the response is
    // already on the wire — poll rather than assume it has landed, the same
    ...
    assert_eq!(
        rows.len(),
        1,
        "exactly the one bound exchange must have been reported, not the earlier unbound one"
    );
    let row = &rows[0];
    assert_eq!(row.provider, "fixture");
    assert_eq!(row.model, "fixture-model");
    assert_eq!(row.route.as_deref(), Some(ANTHROPIC_MESSAGES));
    assert_eq!(row.harness.as_deref(), Some("fixture-harness"));
    assert_eq!(row.outcome, Some(Outcome::Succeeded));
    let dispatched_at = row
        .dispatched_at_unix
        .expect("a real forwarded exchange must record when it was dispatched");
    let completed_at = row
        .completed_at_unix
        .expect("a real forwarded exchange must record when it completed");
    assert!(
        completed_at >= dispatched_at,
        "an exchange cannot complete ({completed_at}) before it was dispatched ({dispatched_at})"
    );
    assert_eq!(
...
fn a_rebind_during_an_in_flight_exchange_is_still_attributed_to_the_binding_that_dispatched_it() {
    ...
    let naming = |rows: &[crate::routing::evidence::NewObservation], harness: &str, model: &str| {
        rows.iter()
            .filter(|row| {
                row.provider == "fixture"
                    && row.model == model
                    && row.route.as_deref() == Some(ANTHROPIC_MESSAGES)
                    && row.harness.as_deref() == Some(harness)
            })
            .count()
    };
    let mut attempts = 0;
    let rows = loop {
        let rows = reported_observations(&seen);
        if naming(&rows, "harness-a", "model-a") > 0 {
            break rows;
        }
        attempts += 1;
        assert!(
            attempts < 200,
            "no routing observation naming the dispatching binding was reported within 2s of a \
             completed exchange"
        );
        std::thread::sleep(Duration::from_millis(10));
    };
    assert_eq!(
        naming(&rows, "harness-a", "model-a"),
        1,
        "the exchange dispatched under binding A must be reported exactly once, as A"
    );
    assert_eq!(
        naming(&rows, "harness-b", "model-b"),
        0,
