...
use crate::provider::budget::BudgetPeriod;
...
///
/// History: design-decisions.md, "Trims: provider/telemetry/mod.rs", fn read_harness_plan.
...
/// `config::QuotaOverride` that holds what the user wrote; this turns
...
/// `provider::resources` to state what a reader would have produced.
///
...
/// for the reason `paths::RuntimePaths::provider_cache_dir`
...
///
/// History: design-decisions.md, "Trims: provider/telemetry/mod.rs", struct GatewayQuotaCache.
#[derive(Debug, Clone)]
...
impl GatewayQuotaCache {
    ...
    ///
    /// What `provider::resources::GatheredTelemetry::gather_gateway_quota`
...
/// `paths::RuntimePaths::project_state_dir`, not
/// `paths::RuntimePaths::data_dir`, the one difference from
...
/// the same `paths::RuntimePaths::data_dir`. `provider/cache.rs`'s
...
/// own `paths::RuntimePaths` and hands this a [`Self::new`] built
/// from it.
#[derive(Debug, Clone)]
...
impl GatewayHealthCache {
    ...
    /// consumer: `provider::resources::GatheredTelemetry::gather_gateway_health`
...
/// frame happened to own it last. `memory::RoutedModel` is what
...
impl DispatchReservationCache {
    ...
    /// `provider::resources::observed_capacity`. A caller that does
