...
//!
//! A discovered catalogue is not configuration. `paths` already
//! separates the two locations, so this uses the one it already has:
//! `paths::RuntimePaths::provider_cache_dir`.
//!
...
impl ModelCache {
    ...
    pub fn new(provider_cache_dir: &Path) -> Self {
        Self {
            root: provider_cache_dir.to_path_buf(),
        }
    }
...
pub fn file_stem(provider: &str) -> String {
