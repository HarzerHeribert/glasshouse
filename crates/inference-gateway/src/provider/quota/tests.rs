use super::*;
use crate::provider::registry::NativeClient;

/// The registry takes its native subscriptions from the caller, so these
/// tests hand it one client of their own rather than naming a real harness.
fn clients() -> Vec<NativeClient> {
    vec![NativeClient::new("client-a", "Client A")]
}
...
fn every_resource_kinds_quota_shape_is_projected_out_of_its_capacity_state() {
    for kind in crate::provider::registry::registry(&clients()) {
        assert_eq!(
            kind.quota(),
            kind.capacity().model(),
