...
pub struct RunArgs {
    ...
    /// arm of [`attempt::FEEDBACK_ARMS`] (`bare,shadow,scout,dissect,lanes,working,outline,oneshot,reduce,prefetch,all`),
...
pub fn expand_pane_feedback(
    ...
    for name in arms {
        let arm = attempt::FEEDBACK_ARMS
            ...
            .ok_or_else(|| {
                format!(
                    "--pane-feedback knows bare, shadow, scout, dissect, oneshot, reduce, prefetch and all, not `{name}`"
                )
            })?;
