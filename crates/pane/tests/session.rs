...
#[test]
fn announcing_regression_tests_does_not_end_the_task_or_execute_the_example() {
    ...
    assert_eq!(
        fs::read_to_string(root.join("regression.txt")).unwrap(),
    ...
    assert!(last_user_text(&requests[2]).contains("completion was not signalled"));
}

#[test]
fn repeated_unmarked_prose_stops_as_incomplete_instead_of_success() {
    let root = scratch_dir("unfinished-prose");
    let rollout = root.join("rollout.jsonl");
    let absent = root.join("no-glasshouse");
    let (base, bodies) = start_fake_provider(
        (0..5)
            .map(|_| assistant_reply("I will do that next."))
            .collect(),
    );
    let output = run_session(
        &root,
    ...
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("without confirmed completion"));
    assert_eq!(
        bodies.lock().unwrap().len(),
        4,
        "bounded continuation, not an unlimited loop"
    );
