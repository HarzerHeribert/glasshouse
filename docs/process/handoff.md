**2026-09-10 (helper/profile repairs ready for release):** Integrated Sol profile picker, full helper accounting and deterministic preparation, plus primary named confined checks. Corrected full Glasshouse workspace and full Pane suite pass; the Native argv regression is fixed without weakening its PTY test (80/80). Next action: finish final Clippy, commit/build/install both binaries, then run fresh v3 Sol/Luna expense repair in the dedicated cmux window and reconcile the frozen oracle, live UI and usage ledger. No new release or capability closure yet. Details: `docs/process/dogfooding-2026-09-10-helper-preparation.md`.
