**2026-09-10 (helper repairs verified; installing and rerunning):** All repair code is integrated and reviewed. Final macOS evidence covers 4,237 Glasshouse/workspace tests through a full sweep plus corrected model-query target reruns, and 890 Pane tests; static/script/build/MSRV checks pass. The original failed intermediate logs are preserved. Next action: commit/build/install both binaries and run `glasshouse-helper-demo-v2` through the subscription profile without `--yolo`, checking the 16-case independent oracle, requested-model/purpose accounting and named-runner reuse. Desktop control currently reports `cgWindowNotFound` for cmux, so the live GUI retest awaits the second window becoming accessible; functional installed-CLI work continues. Details: `docs/process/dogfooding-2026-09-10-helpers.md`. No capability boxes changed.
