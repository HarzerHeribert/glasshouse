**2026-09-10 (helper repair installed; realistic task passes):** `c010eea` is installed for both binaries. A fresh Sol/Luna subscription task completed the expense CLI repair without `--yolo` or operator correction in 361.7 seconds: nine project tests and 16 independent oracle cases pass. Requested-model accounting now separates Sol task turns from Luna helpers. The nested-console evidence fix is integrated and independently accepted; all 894 Pane tests pass. Next action: commit/install the formatter and run a focused real subscription replay. Full macOS validation for the installed batch covers 4,237 workspace tests and 890 Pane tests; failed intermediate logs remain preserved. Desktop cmux access still fails with `cgWindowNotFound`; no new GUI proof is claimed. Details: `docs/process/dogfooding-2026-09-10-helpers.md`. No capability boxes changed.
