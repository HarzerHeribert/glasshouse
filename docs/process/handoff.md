**2026-09-10 (Glasshouse/Pane input repair):** The user's input blockers were reproduced in cmux and repaired by two isolated GPT-5.6 Sol editors with a third Sol reviewer. Embedded launch now enters the harness immediately; viewport clicks restore focus; paste and requested mouse events reach the child; Pane provider/model clicks select without applying; bare `pane` opens its composer. The target state, original findings and live preview are in `docs/process/dogfooding-2026-09-09-input.md`. No capability boxes changed. Next action: install the compiled release and run the user's authorized small real tasks through existing subscription profiles, then record outcomes and the separate quick-open profile-selection gap.
