...
- **State:** `main` at this commit = `ee7b1cd` (pane at `9c4e4ef`: the isolate fix-2, 61F, `pane --version`, the Windows build fix-forward) + the credentials name guard, the README's *Provider keys*, the third dogfooding entry; pushed with this checkpoint. Live: the fifth pane lead (Opus xhigh — the Fable weekly limit read 92 % at 07:22, resets Sep 8; relaunch model parked as `fable-limit-relaunch-model`) inheriting `pane-61e-verify-fix-2`, `pane-tools-reap` (reported), `pane-sigint`; `GH-ATTACH-REVOKED-TERMINAL` (Red, Opus); `GH-GATEWAY-PURPOSE-HEADER` (Amber); `GH-EXTRACTION-CONSENT-NOTICE` reported complete, integrating next.
