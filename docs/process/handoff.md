**2026-09-07 (live structured-return correction):** The visible Gemini subscription session exposed a terminal-control defect after two evidence-gathering cells: Gemini returned `{matchesCount: 0, sampleMatches: []}` as notebook inspection, and Pane promoted that object to the assistant's final answer. Pane now treats objects, arrays, collections and host tool objects as bounded `## Output`, renders them as a distinct notebook region, supplies them to the next provider turn, and reserves ordinary completion for text/scalars; both foreground sessions and `agent.run` use the same predicate. The regression reproduces the exact object and proves a second request plus final prose. Full Pane tests were green except one stale model-picker expectation exposed by the prior active-provider change; that fixture now opens on Google and the complete TUI live target plus Clippy pass. Next action: rebuild the shipped binary and replay the recommendation task visibly through `subscription-gemini-pane`.
