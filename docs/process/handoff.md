**2026-09-11 (standalone bootstrap: the gateway listens before it has a key, and Pane stores one — Fable 5.1):** the first standalone dogfood of the extraction died at start (`pane` in an empty directory, no key exported: the gateway refused to serve with nothing to forward to, its message joined fourteen providers into one sentence, and a Groq key filed in the Keychain by Glasshouse was refused to the gateway — ACL bound to the creating build's code hash, dialogs silenced). User ruling: enter the key in Pane's TUI and have the gateway store it. Landed: `gateway::UpstreamSlot` + `start_awaiting_upstream` (listen first, `503` with the refusal after the bearer check, one rebuild per second, filled once; hosts never deferred, `Gateway::upstream()`/`serving_provider()` now `Option`); `secret::file` — `<data dir>/credentials.toml`, `0600`, store chain file → native → environment; `inference-gateway credentials list|set|remove` (key on stdin, names only out); Pane `/login` lists API-key rows, `/key <provider>` masked modal prompt piped to `set`, start-up notice when nothing is stored. Record and residuals: `design-decisions.md`, the 2026-09-11 addendum under *The inference gateway is its own crate and process*. Next action: install, run the user's exact case (`pane` in an empty directory, `/login`, enter a key, one turn), then a subscription account.
