...
| 1301 | **STALE 2026-09-02 (`GH-RECON-33A-32G`, `phase-32g.md`): translated exchanges write `output_tokens` on served rows since Phase 56, and migration 23 stamps `task_class` on the routing-decision row — the two facts sit on different rows with no join. Ruled 2026-09-02 (evening): the class rides on the gateway's `SessionRouting` state the way the session id now does (`serve_session`), so served rows carry it; a windowed median per class feeds `expected_marginal_cost`'s evidence. Packet `GH-TASK-CLASS-COST-JOIN` (Amber) written, dispatch after `GH-OBSERVATION-SESSION-COLUMN` merges. **CLOSED the same evening** (`phase-32g.md`'s last entry): `serve_task_class` beside `serve_session`, `output_tokens_by_class`, the evidence sentence on both branches of `expected_marginal_cost` — the worker found every classified task takes the tier-established branch, so 1298's dollar figure had only ever been reachable for unclassified runs. |
...
| lines | cause | verdict |
|---|---|---|
| 1347-1352, 1355, part of 1354 | **1350 CLOSED 2026-09-02** (wave 104, `phase-33b.md`: rounds per minute of serving time on the translated path); **1347, 1348, 1349 CLOSED 2026-09-02 (`GH-STREAM-TIMING-MS`, Red — migration 25, an independent Opus verifier accepted; `phase-33b.md`); 1351, 1352, 1355 CLOSED 2026-09-03 (`GH-RESPONSIVENESS-TERMS`: effective TTFC = raw ÷ (1 − failure rate) at the floor, the reliability-adjusted term, the fifth figure on its own line — `phase-33b.md`)** (migration 25: four millisecond offsets since dispatch, the readers, four labelled lines — `design-decisions.md`, *Millisecond offsets on the routing row*); 1351/1352 are `GH-EFFECTIVE-TTFC`'s after it. The wall below is the relay's, not the seam's (batch 91, finding 3). **Cluster L / P1b** — the relay-path body-parsing wall. `gateway/ingress.rs:36`/`:647` and `main.rs:~8612` re-verified word-for-word as still true. TTFC/TTFT/decode-throughput/rounds-per-minute symbols **do not exist in the tree at all** — stronger than Cluster B's built-and-unwired shape, and `shell/state.rs:695` says so in production prose | **REFUSED — needs the `ingress` ruling** |
...
| 1142 | **CLOSED 2026-09-03** (`GH-FILE-AWARE-MEMORY`; `phase-28.md`): advisory by label, stale by commit order, never withheld or rescored. **Design 2026-09-02 (late night), `GH-FILE-AWARE-MEMORY` dispatched** — advisory by label and stale by *commit order* (`git log -1 -- <path>` against `source_commit`), never by reading source or judging conflict; the refusal of a semantic comparison below stands. See design-decisions, *File paths a memory explicitly references*. *"Keep file-aware retrieval advisory and never treat stale memory as stronger evidence than the current source code."* No mechanism anywhere reads current source and compares it to a memory's claim; `memory/policy.rs:280-295` says so in its own doc comment, and comparing a memory against live repository state was declined at 828, 829, 862 and 932. |
...
| lines | missing link |
|---|---|
| 1139, 1140 | **1139 CLOSED 2026-09-03** (`GH-FILE-AWARE-MEMORY`; `phase-28.md`): the firewall hook records `file_touched`, the model names paths, the byte-equality guard keeps only the session's own. **Superseded for 1139 on 2026-09-02 (late night), `GH-FILE-AWARE-MEMORY` dispatched** — Phase 57's context-firewall `PostToolUse` hook (matcher `*`) already delivers every `Edit`/`Write` path to a Glasshouse subprocess; the design records it as a `file_touched` lifecycle event (migration 26) and guards the model's answer against the session's own touched set. `observed` stays `observed`. See design-decisions, *File paths a memory explicitly references*. **No file-path association exists at all.** The `memories` table (`database.rs:303-326`) has no path column — its columns are `id, project_id, kind, authority, status, subject, body, source_session_id, source_commit, superseded_by, created_at, updated_at` — and a tree-wide grep for `file_path\|FilePath\|referenced_file\|associated_file` returns **zero hits**. `memory/extract/` has no path-identification logic. Nothing produces the signal, so there is nothing to retrieve by. Note §71 for 1140 when it becomes reachable: it needs an **enumeration** by path, not a lookup by known id. |
| 1141 | **CLOSED 2026-09-03** (`GH-FILE-AWARE-MEMORY`; `phase-28.md`): `RetrievalIntent::CodeEdit` inside the ladder rung on the file-scoped door. **Design 2026-09-02 (late night), `GH-FILE-AWARE-MEMORY` dispatched** — a `RetrievalIntent::CodeEdit` on `for_path` orders `Constraint`/`Decision`/`FailedAttempt` ahead of the other kinds *inside* the ladder rung; the briefing's file section (built for the files the task names) asks with it. The generic evidence below still does not tick it. **Half already ships and the other half is 1139's blocker.** `memory/inject.rs::briefing` (234-254) already orders invariants, constraints and failed attempts ahead of ordinary matches, tested by `context_injection.rs:820`. But the line sits under a *file-aware* phase and nothing scopes that preference to a file about to be edited. **Do not tick it on the generic evidence** — the phase's premise is the file scoping. |
...
| ↳ P1b, **HISTORICAL — superseded 2026-09-03** | *Context only; not current. The relayed half of this row was reversed by the user's approval above — nothing here still refuses relayed usage.* design-decisions §Phase 56 *"the user's answer on pairs"*: `ingress` keeps relaying served protocols byte for byte and never parses them — but a **translated** exchange (a target the provider does not serve, entering a codec) is parsed by construction, so its usage is recorded as *exact* where the provider states it. P1b therefore opened for translated pairs as `gateway-translate` (T1) landed, and **at that time** stayed refused for relayed ones — **that refusal is superseded: relayed usage reading is APPROVED (2026-09-03), see the ruling above.** Readers must carry tokens where a row has them and say *not exposed* where it does not (`harness-efficiency`). | partly — translated pairs only |
...
| **P7** | a retrieval-quality signal (score computed and dropped, `memory/search.rs:443`) | 1129; 1094 (CLOSED, Phase 24); **939 CLOSED 2026-09-02** (`GH-RETRIEVAL-FEEDBACK` — the rating carries its retrieval's scope, `phase-21f.md`) | no |
...
> **2026-09-03:** RC-C's producers landed during 2026-09-02 (first byte/token/tool-call, tool rounds, repairs, cost, migration 25's ms offsets), and `GH-PHASE51-JOINS` closed **1836, 1855 (token half) and 1854 (sparse + stale)** — `phase-51.md`'s last entry has the three rulings. `GH-RESPONSIVENESS-TERMS` closed **1845** (the pairing-class join, six quantities) and **1850** (the separation readout) the same night. RC-B's *routing* half (1834, 1835, 1837, 1846, 1852) still waits on the product question below. **Answered 2026-09-03 (`.agent-runtime/answers/phase51-routing-rating.txt`): *"Yes, both: explicit rating when given, the turn-outcome proxy otherwise."* 1834, 1835 and 1852 had closed on the proxy by then; **1837 and 1846 are the two lines the answer unblocks**, and their package is `GH-ROUTING-RATING` — a design note in `design-decisions.md` first (the explicit rating's surface and where it is stored, the proxy's precedence), then the implementation. It queues behind Phase 59 (map 2043–2054), which outranks every feature package until its lines close. **A refusal that outlived its blocker is now a package shape — re-read this file's *reasons* after every wave that adds a column or a row kind.**
...
| ~~1367~~ | **CLOSED 2026-09-02 (evening)** by `GH-DISPATCH-RESERVATION-ROW` (Red, Opus 5 high): a per-credential slot file claimed with an exclusive `create_new`, a ten-second lease (`2 × EXTRACTION_BOUND`, pinned), released on completion or drop, netted out of the pool before `choose` through `FreePool::withhold_in_flight` and the router's own `is_available` gate — the two policy classes still do not name each other. `phase-33c.md`'s last entry. Recorded limits: the ceiling it reads is provider-wide (1369's own); a metered candidate is not withdrawn by an empty request pool; the Windows leg was not run (the primitives were chosen to be portable). |
...
## A gate gap the trailing sweep exposed — `blast-radius.sh` did not trace a nested file's parent module (2026-09-02, evening)

A change to `gateway/session.rs` was traced to `--lib gateway::session`; the source-scanning tests in `gateway/mod.rs`'s own `mod tests` run only under `--lib gateway`, so `the_gateway_imports_none_of_the_modules_that_would_make_it_a_harness` went red only in the waves 96–97 sweep (the offending `use crate::session::SessionId` is fixed forward in `GH-TASK-CLASS-COST-JOIN`). **`GH-BLAST-PARENT-MODULE`** (Green, Sonnet low) landed the fix: both mapping loops also push the first path segment of a nested module (`gateway::session` → `gateway` too), pinned by `scripts/tests/test_blast_radius_parent_module.py` (three cases; the mutation was killed by hand because `mutate.sh` parses only cargo's `test result:` line — a Green residue of its own). The same shape exists wherever a parent module keeps scans: `routing/mod.rs`, `routing/disposable.rs`.

**`GH-WATCH-RETRACTS-DONE`** (Green, Sonnet low, 2026-09-02 late): `worker-watch.sh` now retracts a DONE announcement when the pane reads BUSY again — the false nag on a worker resumed with a relayed ruling, captured verbatim (*✻ Mulling… (1m 34s · ↓ 8.4k tokens)*) and reproduced by the lifecycle test; the mutation was killed by hand because `mutate.sh` judges cargo output. The remaining watch residue is the long-thinking false idle, which needs a capture nobody has yet.

## 1275 CLOSED — the token axis of the per-class average, 2026-09-02 (late evening)

Register row P1b listed 1275 as *ingress relay wall*. The wall stands for relayed rows; translated rows have carried tokens since Phase 56 and, since `85b743a`, the launch's task class. `GH-TOKEN-RATE-PER-CLASS` (Amber-light, Sonnet medium) put `tokens_per_hour` on `ClassRate` from those rows and the overview prints it or *tokens not counted* (`phase-32e.md`'s last entry). **Phase 32E is complete.** The P1b row's remaining members (1333, 1263, 1158) are unchanged by this.

## 1436 CLOSED, and the price 1419/1439 waited on now exists — 2026-09-02 (evening)

> **2026-09-03:** **1419 CLOSED** (`GH-LAST-LINES-33C-34B`, `phase-34b.md` — the protected capacity is the launch profile's own backend price, one tenth is *materially lower*, a `protected capacity` ordering term that never excludes; the classifier's sort now sums the verdict's notes). **1366 CLOSED** the same package (`phase-33c.md` — a learned window from the median throttle interval when no header states one; Phase 33C complete). Phase 34B complete.

`GH-CLASSIFIER-PRICE-CEILING` (Amber, Sonnet high): `[routing] max_marginal_cost` — a setting with a settings-surface writer and **no router reader** (Cluster B) — is read by `classification_verdict`, against a ceiling estimate of one classification call at the candidate's `pricing.toml` price; `phase-34c.md`'s last entry. **1419** still needs a producer for *the premium capacity it protects* and a *materially lower* threshold — do not package on the price alone. **1439** needs only the comparison and the ruling now written in `design-decisions.md` (*Preferring a cheap metered classifier over an unreliable free one*); `GH-CLASSIFIER-TIME-PRICE` is its packet, after the price lands. **1439 CLOSED later that night** — the first ruling was proved vacuous by the worker (wasted time ≤ median, and 1435 already excludes on the same limit), amended to a time-against-the-metered-candidate's-own-median comparison, and landed with a shipped-binary proof that it changes a decision (`phase-34c.md`'s last entry). **1440** unchanged (no subscription-backed classifier candidate is ever built).
...
> **2026-09-02, landed in two Green steps:** `GH-ATOMIC-WRITE-UNIQUE-TEMP` (batch 87) gave the primitive a per-writer temporary and loaders that skip `*.writing`; `GH-ATOMIC-WRITE-MAIN-COPIES` (wave 94) routed the two hand-rolled copies in `main.rs` — `ClassificationStickyCache::store` and `ClassificationTextCache::store` — through the now-`pub` `write_json_atomically`, with a race test per store that watches the store's own error log line (a content-only assertion SURVIVED the fixed-name mutation once; the worker said so and strengthened it — 1/1 KILLED after). Neither loader scans a directory, so the leftover-temporary hazard never applied to them. Closed; nothing left here.
...
| — (2014's *recorded reason*) | **`translate::field_rows()` has no production caller** — the pair table's per-field rows, including the new `CacheDisposition`, have never been printed by the shipped binary; the strip's reason reaches a user only through the gateway's opt-in debug record, which is the convention every `Exchange::record` line follows. Not a blocker for 2014 (ruled closed on that convention), but a Green candidate: **`GH-PAIR-TABLE-PRINT`** — `glasshouse gateway pairs` printing `pairs()` with each pair's field rows, one test. **Landed 2026-09-02 (evening), Green, Sonnet low:** `Command::Gateway { Pairs }` → `main.rs::gateway_pairs_report`, `PAIRS` and `FIELDS` blocks (refused with reasons, ignored, `cache:`, `effort:`); `tests/gateway_pairs.rs` 3/3; the one mutation KILLED after the worker tightened a substring assertion that `reasoning_effort:` had been satisfying. `pairs()` and `field_rows()` now have their production caller. |
...
| **1542, 1543, 1544** | `GH-RESPONSIVENESS-TERMS` | CLOSED 2026-09-03: the transport-level failure rate is the second reliability signal 1542's refusal asked for (`observed_pairing_reliability`, clamped to the prior's own magnitude); effective over raw TTFC for tool-using gateway routes; rounds per minute capped at ±0.25 as supporting evidence. 1534/1535/1545 stay refused (no source). `phase-35b.md`. |
...
- **740** (15) — its blocker 745 closed with `GH-WORKER-READ`; prove-it dispatched (`GH-PROVE-IT-BATCH-2`): after the wake-up, the worker session stays `active` and `send_message` still delivers. **CLOSED 2026-09-06** (`phase-15.md`).
...
- **828, 829** (20) — the heuristic refusal above stands; the mechanism is the extraction prompt's own rule (`memory/extract/schema.rs:659`, *could not cheaply rediscover by reading the code*). Prove-it dispatched to pin it, with the limit stated that compliance is the model's. **CLOSED 2026-09-06** (`phase-20-22-and-23.md`).
- **1170, 1175** (31) — uncensused until now; producers found: `routing/session/reserve.rs:246` (*switching and bootstrap cost*) and `launch --from-checkpoint` (Phase 40). Prove-it dispatched. **Both CLOSED 2026-09-06** (`phase-31.md`) — Phase 31 complete.
- **1216** (32A) — the reader is built (`LongWindowRequests`, `window_seconds`); prove-it dispatched; an existing test with a 3 600 s window was the demonstration (**CLOSED 2026-09-06**, `phase-32a.md`). Ticks on that demonstration with the limit that no observed provider sends such a window — distinct from the five Cluster E lines closed as decided-out under decision 5, whose readers do not exist.
