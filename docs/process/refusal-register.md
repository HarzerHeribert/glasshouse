...
| line | missing link |
|---|---|
| 1599 | **Added 2026-08-30 by `GH-QUOTA-HEALTH-ROUTING`, and it is the cluster's clearest measurement.** *"Consider provider health when choosing a destination."* `RouterInputs.health` is a `FreePool`; `launch_session` builds an **empty** one immediately before `SessionRouter::choose` and `main.rs` calls **no `FreePool` mutator anywhere**. All four production `choose` sites do the same. The only live pool is `gateway::session`'s `SessionRouting.free`, whose sole accessor `free_pool()` has one caller inside its own module — no pool ever reaches the router. **Measured rather than argued:** severing `provider_health`'s input SURVIVES against `--test route_command` (the binary's tests, 31 passed) and is KILLED against `--test session_router` (the library's). The scorer is watched; the wiring is not, because there is none. Closing it means bridging `GatewayHealthCache` into `RouterInputs.health` — a design task with two hazards, `FreeResource` keyed by `CredentialId` vs a reading's rendered `credential_label`, and `ResourceHealth::cooling_down_until` an epoch-less `Instant` vs a reading's unix seconds. **The refusal is written at the `FreePool::new()` site in `main.rs` and tripwired by `route_command::a_persisted_provider_health_reading_reaches_the_binary_but_never_the_launch_paths_router`.** |
