...
`GH-CLASSIFIER-PRICE-CEILING` (Amber, Sonnet high): `[routing] max_marginal_cost` — a setting with a settings-surface writer and **no router reader** (Cluster B) — is read by `classification_verdict`, against a ceiling estimate of one classification call at the candidate's `pricing.toml` price; `phase-34c.md`'s last entry. **1419** still needs a producer for *the premium capacity it protects* and a *materially lower* threshold — do not package on the price alone. **1439** needs only the comparison and the ruling now written in `design-decisions.md` (*Preferring a cheap metered classifier over an unreliable free one*); `GH-CLASSIFIER-TIME-PRICE` is its packet, after the price lands. **1440** unchanged (no subscription-backed classifier candidate is ever built).
