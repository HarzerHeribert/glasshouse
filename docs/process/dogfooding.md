...
- **The firewall in `shadow` wrote no `file_touched` row** across a `Read` of `README.md` and an `Edit` of it; `lifecycle_events` holds only `session_started`, `turn_started` ×2, `turn_ended` ×2. The hook was registered (`context-firewall hook --session … --mode shadow` in the harness's settings). Whether `file_touched` is written only for reductions or the shadow path skips it is not decided here — **one probe owed next session**, not a package yet.
