...
- **The firewall in `shadow` wrote no `file_touched` row** across a `Read` of `README.md` and an `Edit` of it; `lifecycle_events` holds only `session_started`, `turn_started` ×2, `turn_ended` ×2. The hook was registered (`context-firewall hook --session … --mode shadow` in the harness's settings). Probed after the session: the matcher is `*`, `is_writing_tool` lists `Edit`, `tool_input_paths` reads `file_path`, and a hand run of `context-firewall hook --session … --mode shadow` with a synthetic `Edit` payload against a throwaway project wrote no row either — and printed nothing under `GLASSHOUSE_LOG=debug`, because the hook process installs no log subscriber, so whichever gate drops the row (`project_relative_path` or `EventLog::append`) warns into nothing. **Defect** — map line 1139's producer does not fire in production; successor `GH-FIREWALL-FILE-TOUCHED` (Amber): make the hook's warnings reach the log, find the dropping gate, and pin it with a test through the hook binary.
