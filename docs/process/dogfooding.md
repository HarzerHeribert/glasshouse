...
- **A configured provider is not a configured account, and the extraction's message points at the wrong one.** After task 2 (edits to `tests/gateway_failure_taxonomy.rs`, `7 passed`), `memory search --path crates/glasshouse/tests/gateway_failure_taxonomy.rs` answered *"No current memories"* again and `evaluation_observations` row 10 read *"none configured (Phase 39 supplies the provider): no provider is configured for Glasshouse's own support work — no model was called"* — with `[providers.groq]` present. `resources` listed no Groq resource; `entitlements` said the true thing: *"No `[entitlements]` entries are configured … Add one under `[entitlements.<name>]` to name an account it may charge."* The unit Glasshouse charges is the entitlement (Phase 56A), and `NoResource::NothingConfigured`'s sentence names the provider instead. **Debt, one line:** the sentence should say *no entitlement names an account for Glasshouse's own work — add `[entitlements.<name>]`*; a Green fix beside the next routing packet. Added `[entitlements.groq] provider = "groq"` on this machine (decision 8) for the rest of the session.
