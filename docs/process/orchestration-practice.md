...
The packet-and-worker cycle pays for the same knowledge three times: the orchestrator reads the code to write the packet, the worker reads it again to start, the orchestrator reads the diff to integrate. Measured 2026-09-17: five commits in six hours in-session with no packets; the night before, one deletion took four worker passes, three hand-applied integrations and two workers dead of context. Rule, from `CLAUDE.md`'s top section: work the session already understands is done here or by a fork that inherits the whole context and edits the main tree — no packet, no worktree, no integration; reading, design and review go to a plain subagent; a cmux worker in a worktree only for a long, parallel, file-disjoint build the user wants to watch, with a short packet. Phase −1, the targeted gate, one mutation per decision and pathspec commits stay at every tier. §§9–10, 73–77 and 84–88 describe the worktree tier and apply only when it is chosen.
