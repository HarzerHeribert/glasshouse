...
> The "Trims: …" sections (comment history moved out of source, 2026-09-05/06) live in docs/history/design-decisions-trims.md.
...
## Pane task tokens are telemetry, not a stopping rule — the user, 2026-09-07
...
## Pane starts on a model the person chose, and its notices are chat history — the user, 2026-09-14

**The report.** A bare `pane` in a new project refused to start; `pane --model opus` started and every request failed, `opus` with a 400 and `claude-opus-5` with a 503 whose text pointed at a broker management endpoint that is switched off; and the startup lines flashed above the prompt and vanished when the UI took the screen. The user: the lines *"should be a part of chat history rather than showing there for a brief amount of time like claude code does"*, and *"not only model but also when settings change etc"*.

**Decided.**
- **No model is still no spend.** The ruling that Pane has no compiled-in model fallback stands. A terminal session without one now opens the model picker and refuses a task until a model is chosen; a scripted session is still refused, before its resume hint is printed.
- **A family word resolves to its newest served model.** A name the gateway's catalogue does not list is read as a word of a model id (`opus`, `sol`) and becomes the newest model carrying it, among accounts holding a login first, then every account; the chat says so. A word several families share (`claude`, `gpt`) is kept as typed rather than guessed. `/model <name>` resolves the same way.
- **A recognised failure says what to do.** A subscription login the provider rejects (`auth_unavailable`, or a 401) names the account and `/login <account>` (the gateway's connect command outside a terminal); an unknown model points at `/model`. The advice is Pane's: the gateway's relay passes bodies through and parses none (`no_part_of_the_relay_deserializes_anything`), and the gateway still reports a login as authenticated until a request fails.
- **Notices are history.** Every session notice and every command outcome (theme, motion, sidebar, status line, settings saved, handlers, errors) is kept in the conversation at the turn boundary where it happened; notes said before the UI exists open the conversation. Two keystroke hints stay transient: "Working…" while a task runs, and panel staging feedback. The two startup lines a terminal's footer already shows (sandbox counts, supervisor off) are not repeated there; scripted output is unchanged.

**Evidence.** `session::tests::{startup_model_requires_a_concrete_cli_or_persisted_choice, a_family_word_resolves_to_its_newest_served_model, settling_keeps_a_listed_id_and_prefers_logged_in_accounts, a_recognised_request_failure_says_what_to_do, the_project_is_named_by_its_folder_not_by_a_relative_root}`, `tests/tui.rs::notices_stay_in_the_conversation_where_they_happened`; four mutations KILLED (a terminal refusing without a model, the oldest family member chosen, notes drawn a turn late, a login failure left unexplained). Driven in a pseudo-terminal against the installed gateway: a bare start opens the picker; `--model opus` notes `opus resolved to claude-opus-5`, and with the Claude plan's refresh token rejected the error reads *"The `claude-max` login is no longer valid. Type /login claude-max to sign in again."*

## A subscription sign-in is the broker's own login, driven from Pane — the user, 2026-09-15

**The report.** `/login` showed the sign-in link as one panel row cut off with "…"; the link could not be clicked or copied whole, a click opened a truncated address in the terminal's own browser, and Claude answered *"Invalid OAuth Request: Missing scope parameter"*. There was no way to sign in on a machine without a browser. The user: *"it opens cmux in app browser not the gateway's callback in Safari and this does not support device code for machines with no browser"*.

**What reading found.** The gateway ran a hand-written PKCE flow and wrote the credential itself, as type `anthropic` or `openai` with no email or ChatGPT account id, while the broker reads types `claude` and `codex` with those fields; Claude's token exchange also wants JSON with the `state` and the Node-like TLS client the broker carries. A login that completed could therefore look connected and serve nothing.

**Decided.**
- **The broker signs in.** `inference-gateway subscriptions connect` runs the pinned CLIProxyAPI's own login (`-claude-login`, `-codex-login`, `-codex-device-login`, `-antigravity-login`) with `-no-browser`, against a private per-login config that is removed afterwards, so the credential is the broker's own format. Only the sign-in link, a device code, the saved account and a failure (status kept, provider body dropped) cross; the broker's SSH hint, which names the machine's public address, and its logs do not.
- **The gateway opens the default browser** with the system launcher (`/usr/bin/open` on macOS), `https` links only, and never over SSH or on Linux without a display.
- **A machine without a browser still signs in.** Any provider: open the link on any device, sign in, paste the address the browser ended on; the gateway forwards stdin to the broker, which accepts it. OpenAI also takes a device code (`--device-code`, `/login <account> device`), and it is the default for OpenAI when no browser can be opened.
...
## Line 2640 has no producer: the shell has no network — the orchestrator, 2026-09-17 01:05

While writing the remote-gate packet: `Profile::grants_network` is always false (sandbox-grants §4.1 — "a network grant would have to be invented"), the seatbelt carries `(deny network*)`, Linux drops the network namespace, Windows measures its isolation. No `ssh` or `scp` can run in a Pane cell in any mode, so "gate remote commands in `explore`" would first need a network door that §4.1 forbids the allow-list from inventing. Recorded in `phase-69.md` (2640) as *awaiting user decision* with a proposal: a host-run `remote` tool outside the cell sandbox, reaching only `[remote] hosts`, through which `ssh`/`scp` lines pass the static parse, the decision model's "only reads" question in `explore`, and a rollout line each. Not built overnight; the user rules in the morning.

## The dispatch reservation went with the router — the orchestrator, 2026-09-17

Map line 1367 (*reserve known paced capacity at dispatch so concurrent workers do not all consume the same apparent allowance*) was a router feature: the reservation row only meant something because the disposable router read it to choose another resource. With the router deleted under *Glasshouse does not route*, its test file `tests/launch_reservation.rs` had been red on every sweep cell and could only go green by re-implementing a router gate. Retired instead (evidence: `phase-33c.md`, *1367 — RETIRED WITH THE ROUTER*). The cache itself, `DispatchReservationCache`, already lives in the gateway crate and stays there without a caller; pacing across processes is the gateway's concern if it ever returns. This is the first line the routing ruling retires by name; the v1 freeze's triage does the rest in one pass.

## The paste grammar lands, gated to the console-record path — the orchestrator, 2026-09-17

`GH-PANE-WINDOWS-CELL` (2026-09-11) reassembled `ESC [ 200 ~ … ESC [ 201 ~` into an `Event::Paste` inside `terminal_input.rs`, because crossterm on Windows reads console records and never produces one itself: without it the markers were typed into the draft and the payload's newline submitted a turn mid-paste (measured on the ARM64 VM). `GH-PANE-WINDOWS-VERIFY` kept it out of the landing because it was not a no-op on Unix: an opener typed by hand with gaps held every keystroke until a closing marker that never came, and `[2` was held where it used to be released. The grammar now runs only where crossterm reads console records (`TerminalInput::new(cfg!(windows))`), with both paths tested on every host through the explicit switch, so Unix input is byte-for-byte what it was and Windows pastes arrive whole. The verifier's two findings are the module's own regression tests. Mutations: the gate forced on, KILLED by the two Unix tests; the closing marker never recognised, KILLED by three grammar tests. The grammar is exercised by its unit tests on every host; the live tests could not exercise it on either Windows leg, for the reason in the next section.

## Bracketed paste does not reach the app through ConPTY — the orchestrator, 2026-09-17

Traced on the Windows ARM64 VM with an event log inside `TerminalInput::accept`: the bytes `\r ESC[200~ o ESC[201~` written to the pseudo-console's input arrived at the app as Enter, Enter, `o`. ConPTY parses the paste markers itself and forwards nothing for them, and crossterm on Windows reads console records, so the app cannot tell a paste from keystrokes there: a pasted `o` accepts an open approval dialog, and a pasted newline submits the line before it. The x64 GitHub runner shows the same two symptoms. The two live tests that guard against exactly this (`live_approval_once_session_and_deny_gate_actual_writes`, `live_composition_completion_model_selection_busy_input_resize_and_exit`) now detect the stripped markers and print `skipped:` for that step instead of asserting what the console has already decided; they keep asserting everywhere the markers arrive. The way through, if the guard is ever wanted on Windows, is not in the grammar: it is reading the console with `ENABLE_VIRTUAL_TERMINAL_INPUT` and parsing the VT bytes in Pane instead of through crossterm's record reader — a Red package of its own, listed for the map triage as a product limit rather than built. The paste grammar stays: it is correct and tested where markers do arrive as keys, which the 2026-09-11 measurement on an earlier build of the VM's Windows saw.

## Pane first, and Pane gets a network — the user, 2026-09-17 (morning)

Rulings on the eight decisions of *The Open Ninety-Four*, in the user's words where they gave them:

1. **Map 2623, Pane without a sandbox on Windows: build it (A).** The unconfined launch exists on Windows as it does on macOS and Linux; a specialist package verified on the ARM64 VM.
2. **Map 2640, remote commands: drop the line as written and redefine (C).** "A good coding harness can traverse and execute commands via ssh execution on machines, collect logs or just run scripts via shell/python. This has shown super useful to me and done right and using a capable model I would not deem it as critical. The user would need passwordless access via keys to machines of course and the user has to configure this themselves — pane can maybe help but this isn't the scope here. Also network calls like fetching content or using a search engine to find results which matter to a user and enrich context with third-party sources is valuable and necessary." So the premise *the shell has no network* is superseded for named tools: Pane gets `ssh` execution against hosts the user has key access to, a fetch tool and a search tool, as host-run tools with their own permissions and a log, not through the cell sandbox. Explore mode's promise stays: there, remote commands run only when they read (the static parse and the decision model's question, as 2640 had it). The new lines are Phase 70.
3. **Pasting on Windows: read the console the other way (B).** Pane reads raw terminal input on Windows (`ENABLE_VIRTUAL_TERMINAL_INPUT`) and parses it itself, which delivers the paste markers and makes mouse reports independent of the keyboard layout. Phase 70.
4. **The "Maybe" block: promote what the user wants, drop the rest (C)** — the user has not yet chosen which candidates; a plain-words list of D, E, G, I, J, K and L with a pick each is owed. Until then nothing moves.
5. **Phase 62: three lines now, eleven deferred (A)** — but under *Pane first* (below) the three are Glasshouse work and wait behind Pane.
6. **Measurement lines: off the map into the measurements document (A)** — applied at the v1 freeze, when line numbers are renumbered anyway (map IDs are line numbers; moving lines earlier would shift every reference).
7. **The Codex tests on the Windows ARM runner: attribute it (A).** The user's guess was an update prompt; the tests launch a fixture script, not the real Codex, and the Windows fixture starts PowerShell to format a timestamp — the ARM runner's cold PowerShell start has exceeded the 20 s bound on every run since 01:22 UTC while the VM passes. The pin: "pinned is also bad, we move with time" — CI's Codex pin (`CATALOGUE_OBSERVED_VERSION`) is to go, with the adapter verified against the moving version; a Glasshouse item, queued behind Pane.
8. **"I want you to focus on pane though and not on glasshouse. As soon as pane is finished and polished we can build the orchestrator."** Pane first: every package until further notice is Pane's; Glasshouse gets fix-forwards for red cells and nothing else.

## In-session work first — the user, 2026-09-17

"I have a feeling this whole writing packets and using workers and rebuilding context thing is highly ineffective. Do you have another idea? … Just use subagents?" — and, on the three-tier answer, "I like your new approach, makes total sense; make sure future sessions never fall back to the inefficient work SDLC." The rule and its measurement are the top section of `CLAUDE.md` (*How work is done*): in-session or a fork by default, a plain subagent for reading, design and review, a worktree worker in cmux only for a long, parallel, file-disjoint build the user wants to watch, with a short packet. Kept at every tier: Phase −1, the targeted gate, one mutation per decision, pathspec commits, the VM leg for Windows. Retired: packets for everything, the two-worker floor, pipeline nagging, per-worker watches and `integrate.sh` as defaults, and the visible-workers-only rule of 2026-08-25. The first work under the rule: the network-tools design as a subagent and the Windows sandbox bypass as a fork, the same hour.

## The network tools: rulings on the design's five questions — the orchestrator, 2026-09-17

`docs/product/pane/network-tools.md` (GH-PANE-NETWORK-TOOLS-DESIGN, a subagent under the in-session rule) found that `fetch` and `search` already exist as host globals `web.fetch`/`web.search` through a host-owned broker with a domain policy (`crates/pane/src/web.rs`), configured from `[web]`; map 2656 and 2657 are finishing lines, and `NEVER_REGISTERED` constrains only `registry::ALL`, so it needs one sentence, not a rewrite. Rulings: (1) keep `[web]` and add `[ssh]`; no `[network.*]` rename. (2) An empty `allow_domains` refuses — the user's "refused until configured" — the one behaviour change to shipped code; `[web].enabled` defaults false so nothing shipped relies on the permissive reading. (3) Brave Search is the first keyed provider, SearXNG the keyless one. (4) A search key reaches pane from its process environment first and the gateway's `credentials.toml` second; never `.pane/config.toml`, argv or the rollout; no `credentials get` is added. (5) `ssh.run` in `execute` runs any command on a configured host, as the user ruled; a per-host `read_only = true` applies the static parse in every mode for whoever wants it; `scp` is refused, not omitted. Order: fetch first (it owns 2658's gate), then search, then ssh.

## Raw terminal input on Windows — the orchestrator, 2026-09-17

With `ENABLE_VIRTUAL_TERMINAL_INPUT` set on the input handle (`session/ui/console_mode.rs`, after raw mode, cleared on restore), ConPTY delivers arrows, function and modified keys and SGR mouse reports as translated console records, so a mouse report no longer depends on the keyboard layout and the AltGr rule becomes the fallback for a console that refuses the flag. Bracketed-paste markers are consumed by ConPTY under every input mode measured (record and VT), so the paste half of the user's decision 3 is a Windows console limit, not Pane's: the two paste guards keep their `skipped:` branches there, and map 2654 was redefined to the half that holds. The flag also made the parser the design expected unnecessary: crossterm already translates the records.

## What Pane takes from the other harnesses, and providers by configuration — the user, 2026-09-17

On the five ideas drawn from oh-my-pi and its peers: "no, I don't like adding roles to model — with a new one coming out every week this gets messy and necessitates subscriptions people might not have"; "just take into Pane what you think is worth building and not what you think could be a gimmick only". Taken, as Phase 72: diagnostics after every write filtered by the decision model; a symbol index and transactional rename through oxc, never an LSP process; step completion decided by the decision model; steering a running task through the inbox. Dropped: model roles per mode, LSP server and DAP integration, the apply-model pattern (Phase 71's tags cover it), browser control before fetch and search. On providers: "oh-my-pi supports like 60 providers; Pane's gateway has some routers hardcoded I think, which is bad — addable by default and support more". The gateway already reaches any Anthropic- or OpenAI-compatible endpoint from `[providers.<name>]` (`config.rs`: `base_url`, `protocol` in anthropic-messages, openai-chat, openai-responses); Phase 73 adds the presets, model discovery and adding a provider with its key without editing TOML. Protocols that need cloud signing (Bedrock, Vertex) are not taken.

## No Gemini subscription reuse — the user, 2026-09-17 (evening)

Asked how oh-my-pi reaches sixty providers (a few wire protocols over a data table, the vendors' own OAuth flows re-implemented, and other CLIs' credential stores read), the user ruled on the one that matters: "Gemini deliberately prohibits third-party use and it can lead to account freezing." So Phase 73 takes keyed APIs only: Gemini through its paid API key on Google's OpenAI-compatible endpoint (no `google-generative-ai` protocol, no `.gemini` credential reading, no Antigravity login), and no Copilot login either, whose terms restrict third-party clients the same way. The gateway's existing subscription broker for Anthropic and OpenAI logins stays as the user's own choice; nothing new of that kind is added.
