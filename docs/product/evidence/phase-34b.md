...
## 1419 — CLOSED 2026-09-03 (`GH-LAST-LINES-33C-34B`, Amber, Sonnet high): the premium capacity a classifier protects — Phase 34B complete

**Design:** `design-decisions.md`, *The premium capacity a classifier protects*: the protected capacity is the launch profile's own backend — what the task lands on when classification does nothing — priced through `PriceTable::price_for`; *materially lower* is one tenth. The worker followed the note over the packet's paraphrase (which said *the priciest of `site.destinations`*): in `launch_session` the fresh destination whose `launch_profile()` is the profile being launched is priced and threaded as one `Option<ModelPrice>` down `classify_for_routing` → `classify_with_routing_model` → `automatic_classification_model` → `automatic_classification_choice` → `ClassificationPolicy::with_protected_capacity_price`. `routing/disposable.rs :: protected_capacity_note` in `classification_verdict`, after the 1436 ceiling: `+1.0` at or under one tenth (`estimated_classification_cost_micro_usd` at each price, the same token counts), a bounded negative magnitude above it — **an ordering term, never an exclusion**; `0.0` with the reason for a free candidate, an unpriced candidate, or an unpriced protected backend.

**The one decision the worker made, read and accepted:** `choose_for_automatic_classification`'s sort summed only the four preference contributions and discarded the verdict's notes; every note was `0.0` by construction, so 1419's term as literally specified would have been a label with no power to change who wins, and the packet's own test 6b could not pass. The sort now sums the notes too (`admitted.sort_by`'s `of()` chains `notes`) — behaviour-preserving for every existing line and what makes 1419 order. `ratio-inverted`, `term-excludes`, `unpriced-read-as-free` KILLED. Recorded: `glasshouse route --explain` is not a flag — the default `route` output prints every contribution's evidence.

Gates and report: the 1366 entry in `phase-33c.md`. State: **COMPLETE**. **Phase 34B stands at 15 of 15.**
