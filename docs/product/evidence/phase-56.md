...
### Live closure of the subscription-broker producer — 2026-09-07

The Phase 56/56A evidence above described the entitlement model, routing rules, protocol codecs and fixture gateway, but the production gateway still had no caller that could turn a subscription OAuth account into an upstream. The checked map statements at lines 1948 and 1969 therefore overstated the shipped behavior. Commits `97b5946`, `62e9d1e`, `b2d95fc`, `0f939c3` and `7fb91fa` close that missing producer with a managed, entitlement-isolated CLIProxyAPI sidecar. Glasshouse owns OAuth and stable auth directories; a harness receives only Glasshouse's disposable loopback key. One sidecar serves one exact entitlement, inner retries/account switching are disabled, the stored entitlement is reused on resume, and an unpinned gateway profile keeps its prior API-provider behavior.

State: **COMPLETE after live proof.** Three real Pane launches traversed the shipped binary, Glasshouse gateway, account-specific broker and provider subscription, and each returned its exact marker:

- session `9b98f0dff9d6`: `gemini-subscription` / `gemini-3.8-flash-high` → `GLASSHOUSE PANE GEMINI OK`
- session `7c0b32872b7d`: `claude-max` / `claude-sonnet-4-6` → `GLASSHOUSE PANE CLAUDE OK`
- session `b1ffd6f222db`: `chatgpt-subscription` / `gpt-5.6-sol` → `GLASSHOUSE PANE OPENAI OK`

Live dogfooding found two defects the fixture process did not: Rust string continuation had flattened nested YAML, and an authenticated HTTP 200 arrived before CLIProxyAPI had registered any models. `7fb91fa` renders exact nested YAML and requires two stable authenticated `/v1/models` responses with a nonempty catalogue. The regression suite covers malformed nesting, an empty-model 200, a rogue listener, early exit, per-entitlement separation, outer-to-inner key replacement, exact Anthropic tool bytes, JSON and SSE relay, and broker cleanup.

Validation: `cargo test -p glasshouse --bin glasshouse -- --nocapture` 114/114; `cargo test -p glasshouse --lib gateway::subscription_broker -- --nocapture` 6/6; `cargo test -p glasshouse --test subscriptions_cli -- --nocapture` 2/2; `cargo clippy -p glasshouse --all-targets -- -D warnings`; independent verifier ACCEPT. No OAuth token value was read, printed, placed in a project file or handed to Pane.
