...
## 1351, 1352 and 1355 — CLOSED 2026-09-03 (`GH-RESPONSIVENESS-TERMS`, Amber, Sonnet high): effective TTFC, reliability-adjusted comparison, and the fifth figure on its own line

**Effective TTFC (1351).** `routing/evidence.rs :: RouteResponsiveness` (a pure function over `&[RoutingObservation]` for one provider/model, and `effective_ttfc_ms` on `PurposeConsumption` for the readout): `raw_ttfc_ms / (1 − p)` where `raw` is the mean `first_tool_call_ms` and `p` the route's transport-level failure rate over the same window, defined only when both samples are ≥ `MIN_SAMPLE_FOR_SUMMARY` and `p < 1`, `None` otherwise — never a clamped number. `effective-is-raw` and `floor-dropped` KILLED.

**Reliability-adjusted comparison (1352, with 35B's 1543).** `routing/session.rs`'s `responsiveness (effective TTFC)` term: for tool-using work the candidate set's best effective TTFC scores `+1.0` and each other candidate `best / own`; inert with the reason below the floor or for a workload that is not tool-using. A fast-and-flaky route (800 ms, failure rate 0.5) loses to a slower sound one (1200 ms, 0.0) — `routing_score::a_reliable_slower_route_beats_a_fast_flaky_one_on_effective_ttfc_for_tool_using_work`.

**The five figures separate (1355) — ruled closed.** The Red package left this line open on its *effective TTFC* clause; `render_routing_cost` now prints `effective TTFC (reliability-adjusted)` on its own line directly after `TTFC`, `(below floor)` per group, plus `<ms> of <n> measured` beside each latency mean (the verifier's finding on the Red package — the ms sample counts had never been printed — with a mixed-group test). `headline-merged` (the `TTFC` line's newline dropped so the next figure lands on it) KILLED by three tests including `routing_cost::five_reliable_rows_print_a_real_effective_ttfc_on_its_own_line`; the structural scan `no_line_of_the_routing_cost_readout_carries_two_of_the_four_figures` still holds over the rendered report.

**Gates:** routing_score 9/9, responsiveness_terms 4/4, pairing_prior 22/22, routing_cost 14/14, routing_outcome 4/4, routing_economics 22/22, session_router 19/19, interactive_score_terms 7/7, `--lib routing` 286/286, `--lib evaluation` 6/6; targeted blast exit 0; fmt/clippy/doc clean. **Recorded limits** (the worker's): `PurposeConsumption` gained two additive SQL columns for the failure-rate half (`failure_rate_sample`, `failed_count`) — an edit inside `consumption_by_purpose`, read as within "new items only"; `responsiveness_separation`'s effective-TTFC measure can carry a smaller sample than the other three (a row's effective value depends on its route clearing the floor); macOS only. Report: `.agent-runtime/report-responsiveness-terms.md`.

State: **COMPLETE** for 1351, 1352 and 1355. Phase 33B stands at 11 of 14 (1353/1359, 1354's half, 1356/1360 refused).
