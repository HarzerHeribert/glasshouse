...
**Defect found, pre-existing, not fixed here (Red successor `GH-PANE-HARD-LINKS`):** a hard link planted under the root — e.g. `.pane/scratch/hard` → `.pane/config.toml` — is judged by its path in `Profile::check` and in seatbelt, so a confined write through it rewrites the target. Measured by the scratch-plan worker against a real seatbelt. Applies to every never-writable file, in every mode, before and after the carve-out.
