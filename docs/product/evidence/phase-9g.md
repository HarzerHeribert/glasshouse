...
Limits: (1) **the harness's hook subprocesses inherit the stripped environment** — memory extraction and the context-firewall reducer resolve a configured provider's credential keychain-first and environment second, so an environment-only key no longer reaches them while a keychain key still does; by decision 11 the keychain is the supported place, no credential channel is built, and the loss is loud in three places — the extraction hook's stderr line, the firewall hook's stderr line and `glasshouse doctor` — each naming the provider and the variable, never a value, with the platform's store command (`security add-generic-password -s glasshouse -a <VAR> -w`; Windows Credential Manager `<VAR>.glasshouse`). (2) A native launch loses a harness's own sign-in variable only when a user names it explicitly in `credential_env` — no built-in template defaults to `ANTHROPIC_API_KEY` or `OPENAI_API_KEY`. (3) The hook notices are proven through their seams, not by running a hook inside a real harness child. (4) The Linux instruction's attribute is `username`, corrected at integration from the verifier's Debt 1. (5) macOS is the proven leg; the Windows and Linux cells are the trailing sweep's.
