...
## 488 — re-ticked 2026-09-06: every launch site strips every configured provider's credential from the harness child

State: **COMPLETE** — `GH-LAUNCH-STRIPS-PROVIDER-CREDENTIALS` (Fable, Red — secrets; report `.agent-runtime/report-launch-strips-provider-credentials.md`), verified by an independent read-only Red verifier (`.agent-runtime/report-verify-launch-strips-provider-credentials.md`: REJECT on a consequence the packet had not named, then ACCEPT after fix-forward 2). Ruled by the primary; the policy is the user's decision 11 (`design-decisions.md`, *Steering decisions of record — 2026-09-06*).

Contract: given a configuration whose providers name credential environment variables, when any production path starts a harness, none of those variables is present in the harness child's environment even when the launching shell exported them, while preserving that an overlay's own `env` still wins per key, that no value is read or logged, and that a launch with zero configured providers removes nothing.

Production: `launch.rs :: HarnessLaunch::without_provider_credentials` (every `provider_names()` entry's resolved `credential_env`, template defaults included, deduplicated, through the existing `env_remove` seam) at all five production `HarnessLaunch::new` sites — `commands/launch.rs :: launch_session`, `commands/resume.rs :: resume_session`, `shell/mod.rs :: start_session` and the embedded resume, `api/unix/sessions.rs :: spawn` — each before any overlay applies.

Tests: `pty_smoke::a_configured_providers_credential_never_reaches_the_launched_harness` (the shipped binary under a real pty; the variable absent, a control sibling present, the value nowhere); `launch::tests::{configured_provider_credential_names_are_removed_from_the_child, an_overlay_env_after_the_strip_wins_for_its_own_key_only, no_configured_providers_strips_nothing, every_production_harness_launch_site_strips_provider_credentials}` (the last a source scan over the four files asserting exactly five sites). The consequence for the harness's hooks (below) is proven at its seams: `tests::a_withheld_provider_credential_is_named_by_the_extraction_notice_and_never_its_value`, `tests::the_firewall_reducer_notice_names_the_withheld_variable_and_never_a_value`, `integrations::tests::doctor_names_an_environment_only_provider_credential_the_harness_cannot_see`.

| mutation | change | result | killed by |
|---|---|---|---|
| `credentials-not-stripped` | the strip call removed at `launch_session` | KILLED (re-run by the verifier) | the pty test |
| `strip-loop-dropped` | the `env_remove` loop replaced | KILLED | `configured_provider_credential_names_are_removed_from_the_child` |
| `shell-resume-site-not-stripped` / `api-spawn-site-not-stripped` | one site's strip removed | KILLED | `every_production_harness_launch_site_strips_provider_credentials` |
| `extraction-notice-reverted` | the `Model(Unavailable)` arm of `lost_extraction_notice` removed | KILLED (re-run by the verifier) | `a_withheld_provider_credential_is_named_by_the_extraction_notice_and_never_its_value` |

Limits: (1) **the harness's hook subprocesses inherit the stripped environment** — memory extraction and the context-firewall reducer resolve a configured provider's credential keychain-first and environment second, so an environment-only key no longer reaches them while a keychain key still does; by decision 11 the keychain is the supported place, no credential channel is built, and the loss is loud in three places — the extraction hook's stderr line, the firewall hook's stderr line and `glasshouse doctor` — each naming the provider and the variable, never a value, with the command that stores it: **`glasshouse credentials store <VAR>`** (`GH-CREDENTIALS-STORE-VERB`, 2026-09-06 — the `security add-generic-password` instruction shipped first was found unreadable by Glasshouse in the day's second dogfooding session: on macOS an item's access list names the program that created it and Glasshouse never shows the Allow dialog, so only an item Glasshouse creates is readable by Glasshouse; `doctor` now tells *present but not readable by Glasshouse* from absent and names the removal command). (2) A native launch loses a harness's own sign-in variable only when a user names it explicitly in `credential_env` — no built-in template defaults to `ANTHROPIC_API_KEY` or `OPENAI_API_KEY`. (3) The hook notices are proven through their seams, not by running a hook inside a real harness child. (4) The Linux instruction's attribute is `username`, corrected at integration from the verifier's Debt 1. (5) macOS is the proven leg; the Windows and Linux cells are the trailing sweep's.
