...
## 1367 — RETIRED WITH THE ROUTER 2026-09-17 (the user's ruling of 2026-09-16)

The reservation row (`GH-DISPATCH-RESERVATION-ROW`, 44a3f011) was "netted out of the pool through the router's own gate": a dispatch reserved the request it was about to spend, and the disposable router chose another resource when the last one was spoken for. The router is deleted (9395a98e; *Glasshouse does not route*), and the memory-extraction dispatch no longer chooses among resources, so the four `tests/launch_reservation.rs` tests that drove two real `glasshouse memory commit` processes against one canned upstream had nothing left to observe: red on every sweep cell since the router went (4 of 6, red alone too). Retired: `tests/launch_reservation.rs`, the lib companion `the_reservation_lease_outlives_the_extraction_it_covers`, and the fixture's two `check-secrets-allow.txt` rows. `DispatchReservationCache` stays where it lives, in `inference-gateway::provider::telemetry`, with no production caller: if paced capacity is ever reserved across processes again, it is the gateway's feature and the cache is its start. The map line keeps its tick as history; the v1 freeze's triage lists it as *removed with the router*.
