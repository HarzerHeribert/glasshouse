...
State: **IN PROGRESS on `pane/integration`** (2026-09-05). Red tier: Opus specialist plus an independent verifier, platform code on three operating systems. The grant compiler and pre-call path check (`GH-PANE-61D-PROFILE`, `b4046ff`, `crates/pane/src/sandbox/profile.rs`) reached `main` with the 61C merge, unwired; the three appliers (`GH-PANE-61D-APPLIERS`, `7a06298`/`15e7b14`) and the fixes for the two escapes the first verifier found (`ec74850`) are on the lead's branch with a second verifier over them. Rulings of record, each in its file under `.agent-runtime/pane/`: `ruling-61d-exec-roots.md` — exec is granted on the resolved binary, not on directories, wired at 61E where a spawn path exists; `ask-primary-sandbox-4-2.md` — the machine's credential and identity store is never writable, now in `sandbox-grants.md` §4.2, and §4.3's title is the rule; `ask-primary-61d-scope.md` — the `pane` CI job gained a `windows-latest` cell, and **line 2456 is a Glasshouse-side package, `GH-PANE-2456-CONTAMINATION`** (Red), shaped like `tests/pane_launch.rs`, driving the built `pane` binary as the actor behind Phase 46's four contamination tests unchanged; it dispatches after the appliers merge, and 2456 stays open until it lands.
