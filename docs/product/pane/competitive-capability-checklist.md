...
| Completely unrestricted mode | skip permissions | full access | 🟡 | Linux now has explicit `--yolo --dangerously-bypass-os-sandbox` for externally isolated benchmark/CI containers. It bypasses Pane's OS layer but retains admission checks and credential stripping, so it is deliberately narrower than unconstrained host execution. |
