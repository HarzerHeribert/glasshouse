...
| Completely unrestricted mode | skip permissions | full access | 🟡 | `--full-access` is one flag on macOS, Linux and Windows: the `--yolo` admission profile, the `full` rung, and no OS confinement of Pane's own. It bypasses Pane's OS layer but retains path admission, the deny patterns and §4's never-grantable set, so it is deliberately narrower than unconstrained host execution. |
...
| Full autonomous/yolo mode | ✅ | ✅ | 🟡 | **P0** | `--yolo` remains sandboxed by default. Linux can pair it with the explicit dangerous OS-sandbox bypass when an outer container/VM is the boundary; project config cannot enable that path. |
...
| Environment/capability manifest before acting | implicit | implicit | ✅ | **P0** | `## Environment` block from the compiled profile: roots, reserved paths, command policy, absent executables, unavailable capabilities. [container_profile](../../../crates/pane/tests/container_profile.rs), [session](../../../crates/pane/tests/session.rs). |
| Atomic multi-hunk and sequential same-file edits | ✅ | ✅ apply_patch | ✅ | **P0** | `edit({olds, replacements})` applies together or not at all; a following edit binds to the version Pane produced. [mutation_composition](../../../crates/pane/tests/mutation_composition.rs), [exact_edit](../../../crates/pane/tests/exact_edit.rs). |
| Per-call outcomes for parallel tool calls | ✅ | ✅ | ✅ | **P0** | A multi-call direct frame isolates each call; a thrown cell reports completed effects. [direct_frame_outcomes](../../../crates/pane/tests/direct_frame_outcomes.rs). |
| Delta observations instead of repeated inventories | n/a | n/a | ✅ | **P1** | Changed handles in full, unchanged ones as one line; the inventory stays exact. [observation_delta](../../../crates/pane/tests/observation_delta.rs). |
| Evidence-gated completion with final-state checks | ❌ | ❌ | ✅ | **P0** | Stray build artifacts, misplaced coverage trees and stale verification hold a terminal return once; unverified completion is recorded. [final_state_contract](../../../crates/pane/tests/final_state_contract.rs), [task_capsule](../../../crates/pane/tests/task_capsule.rs), [no_progress](../../../crates/pane/tests/no_progress.rs). |
| Interface-origin and recovery-cost telemetry | partial | partial | ✅ | **P1** | Provider-selected interface, frame origin, failure taxonomy, request causes, observation and reduction figures in machine output. [telemetry_taxonomy](../../../crates/pane/tests/telemetry_taxonomy.rs), [session_output](../../../crates/pane/tests/session_output.rs). |
| Matched interface ablation runner | n/a | n/a | 🟡 | **P1** | `pane ruler run --pane-interface` and the adapter's `interface` option exist with interface-regret reporting; no model-backed campaign has run yet. [interface_regret](../../../crates/pane/tests/interface_regret.rs). |
