...
<div><h3>A ChatGPT or Claude subscription</h3><p>Start <code>pane</code> and type <code>/login</code>. Choose ChatGPT or Claude: a browser opens to sign in, and on a machine without one — a server, WSL — you get a code to enter on any device. Nothing to configure first.</p>${code(['$ pane', '> /login', '  Sign in with a subscription', '    ChatGPT · Plus, Pro', '    Claude · Pro, Max'])}</div>
<div><h3>An API key, or your own endpoint</h3><p>The same <code>/login</code> lists the providers that take a key; <code>/key anthropic</code> takes one without echoing it. <strong>Custom endpoint</strong> asks for any OpenAI- or Anthropic-compatible URL and its key — a local model server, a company proxy.</p>${code(['> /login custom', '  Endpoint base URL: https://llm.example.com/v1', '  What it speaks: (Enter) OpenAI-compatible', '  API key: ••••••••'])}</div>
</div></section>`;
